...
function IndividualRoom() {
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      console.error(
        "Error creating booking before payment:",
        error
      );
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="lg:w-1/2 relative">
            ...
            {images.length > 1 && (
              <>
                <button
                  onClick={prevImage}
                  className="absolute top-[40%] left-3 bg-black/50 text-white px-3 py-1 rounded-full"
                >
                  {"<"}
                </button>
                ...
                <button
                  onClick={nextImage}
                  className="absolute top-[40%] right-3 bg-black/50 text-white px-3 py-1 rounded-full"
                >
                  {">"}
                </button>
                ...
                <div className="flex justify-center mt-3">
                  {images.map((_, idx) => (
                    <div
                      ...
                      className={`w-3 h-3 mx-1 rounded-full cursor-pointer ${currentIndex === idx
                          ? "bg-blue-600"
                          : "bg-gray-400"
                        }`}
                    ></div>
          ...
          <div className="lg:w-1/2 flex flex-col justify-between">
            ...
            <h1 className="text-4xl font-bold text-blue-900 mb-4">
              {roomData?.type
                ? roomData.type.charAt(0).toUpperCase() +
                roomData.type.slice(1)
                : "Room"}{" "}
              Room
            </h1>
            ...
            <p className="text-gray-600 font-bold mb-4 text-lg">
              Price: ₹{roomData.rate}/night
            </p>
            ...
            <ul className="list-disc list-inside text-gray-600 mb-6">
              {(roomData.room_features || []).map((f, i) => (
                <li key={i}>{f}</li>
              ))}
            </ul>
            ...
            <ul className="list-disc list-inside text-gray-600 mb-6">
              {(roomData.room_policies || []).map((p, i) => (
                <li key={i}>{p}</li>
              ))}
            </ul>
